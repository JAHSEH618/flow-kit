...
FLOW_MICRO_PROD_LINES=0                # 微改通道收**生产行**的上限(1.1.0 / B2;0 = 关,行为同 1.0.x:有生产条整批 RED;设 16 开)。开着时 kind=生产 的文件三条齐才收:
#                                        触到它的每份 patch 头都有 `# repro: <命令> → RC=<n> [末行含 <子串>]` 与 `# ev:<名>` · 面内 · 生产新增合计 ≤ 此数;
#                                        申报行尾 `# 裁决-N micro prod`,末行印「生产 N 行 ⟹ 须派 ④」(④ 只审这些行);缺一 ⟹ RED 走 ③。
#                                        --apply 落笔后 kit 复跑每条 repro(flow-ev … 微改 micro-N-k),RC / 末行子串任一不符 ⟹ 整流回退(B3),不再让编排方手跑
