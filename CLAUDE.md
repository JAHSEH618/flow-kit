...
- `bin/` —— 全部命令,前缀 `flow-`,plugin 启用后进 Bash PATH,按名调用不写路径。每个脚本头注 = 它的说明书。轮级过渡 `flow-round open|close|state`、步骤账 `flow-step`、收口 `flow-close --wrap|--ship`(0.6.0)、规则书退役 `flow-rulebook`(0.7.0)。
