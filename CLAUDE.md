...
3. kit 自己定义的枚举与输出词(快照 / 活树-独占 / 禁入,RED / OK / FATAL)保持中文,是协议的一部分,不进 config。生成段标记 `<!-- flow:gen-begin -->` / `<!-- flow:gen-end -->` 同理:`flow-dispatch` 打、`flow-doc-budget` 认,改一边就要改另一边(预算判的是**自写字节** = 总字节 − 生成段)。
