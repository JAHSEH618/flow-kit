...
按行列出将写的文件与将执行的命令:`flow-init --ws … --repo … --ledger … --spec-dirs … --untracked … --main …`,然后 `flow.config.sh` 里将填入的 `flow_gates` 与 `FLOW_FACT_LINT_ROOTS`,以及 git shim 落点(已有 hook 则追加一行)。一次 AskUserQuestion:照这个执行 / 去掉部分 / 只存档不执行。

## Phase 3 · 执行

1. `flow-init …`(打印每项 已有/已写)。
2. 把用户确认的门写进 `flow.config.sh` 的 `flow_gates`(用 Edit 改注释块为真实函数),填 `FLOW_FACT_LINT_ROOTS`。
3. `flow-config --check` 必须 RC=0;`flow-route-debts --lint` 必须 OK。
