...
②A / ②B / ④ 都用这份(1.1.0 起 ④ 一轴,只审 diff:面 = ③ 改动 + 微改生产行,形制 = 旧 ④B;三态表归 ①写 丁栏,审轮不填)。回件边写边落。开工先读上一轮回件〇节表,按表跳读;整读一个文件要在甲栏给一行理由;**多条读合进同一个 Bash**。
...
- **「做不到 / 结构上够不着 / 不可达」型断言**(②A;④ 只对 diff):必须主动找反例,并把找过的路径逐条列出(直呼导出函数 / 注入端口替身 / 别的调用方 / 别的观测量);推不出反例才判成立,并写明射程限定。
- **④ 对面内行段自设变异**(派单「必查行段」= ③ 〇表第二列原样 + 申报 prod 行):改轮 / 微改自造的盲区是这一轮最可能漏的;门整跑一次;复跑 ② 丙栏的 oracle(kit 已在 `.evidence/微改-log.tsv` 跑过的 micro-* 不重跑)。每条变异开跑前对冻结件 `shasum -c`,非 OK 行 > 0 拒跑。三态表不归 ④(①写 丁栏出)。
