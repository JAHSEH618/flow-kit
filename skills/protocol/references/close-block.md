...
```
0. 收口同会话(1.1.0):flow-batch next 末行印 `收口` 与整行 flow-close --wrap,照抄;请求数超 FLOW_TURN_CAP(flow-usage WARN)才 flow-round state 刷完事实段另起会话
1. flow-close --wrap <流程目录> <基线> <编排方申报清单(含账本 / spec plan / 根文档)> [after-hashes] [--task] [--plan] [--verdicts <三态表>](不给默认取 ①写 丁栏抽的 .verdicts-①写.md)
...
5. 规则书(条件触发):wrap 报「规则书变长」才 flow-rulebook show 看行号 → flow-rulebook retire <行号>[.<子句号>] --section "<§X · 批次>" --reason "<一句>"(逐字搬,不经 shell);没变长这步没有
6. commit(消息 = 〇 正面结论 + 裁决号段 + 门 + 账)—— 单独一条命令,前面不串别的
7. flow-close --ship <commit> --subject "<PR 标题>" [--verdicts <三态表>] --dir <流程目录>
...
8. 与 7 重叠做:memory(号段 / 门 / 进度 / 下一批从 N 起)
```

## 轮数账(每批;全由 flow-usage 生成,零手填)

轮次行(轮数 / 携带 / 输出 / 当量 / 调用每轮 / 读批量 / 模型)+ 墙钟归因(生成 / 工具 / 空等 / 卡顿)+ 固定开销(开工 / 轮间 × N / 收口 / 门整跑 · 缓存命中 / 独占合计)+ 编排方输出去向 + WARN(请求上限 / 读批量 / 卡顿 / 中断税 / 独占超线)
—— 都从 transcripts 算,窗从开工序起到收口末(--ship --dir 之后重跑才含 commit / PR)。手填曾把开工 11 min 估成 35、收口 16 估成 40,所以退役;1.1.0 起「本批退役规矩数」那一格也退(退役是棘轮条件触发,不是每批仪式)。
`.usage.md` 的生成段由 `<!-- flow:gen-* -->` 括起,重跑保留标记之后的手填段(留给编排方随手记,kit 不读)。

## 检验判据(不达标只许退役规矩或修工具,不许加规矩)

- 编排方独占关键路径 = 开工 + 轮间 + 收口 ≤ 30 min(flow-usage 全窗算;p4e 实测 35 = 9 + 11 + 13。旧窗只盖住编排方 46% 的请求,曾报 14 ✅);1.1.0 目标 ≤ 10 min / ≤ 12 请求(flow-batch 三个点)
- 轮内门整跑每轮 ≤ 1 次 + 收口 1 次(旧判据「总数 ≤3」的结构下限就是 4,判据本身错了)
- 不起独立收尾轮;② 后审方的 patch(可含生产行,上限 FLOW_MICRO_PROD_LINES)由 `flow-batch next --verdict <N>` 经 flow-micro 落笔并复跑 oracle;零阻塞 ⟹ 跳 ③,有 prod 行才起 ④(只审 diff),否则直接收口;不起 ③改二
