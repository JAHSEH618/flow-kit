...
```
1. 账本:拿①的追写草稿 + ④的三态表做 diff 审 —— 还 → status:closed;追写 → 正文加「> **<日期>」段;不动 → 零改。
...
8. flow-clear-map-debt --verdicts <④三态表文件> <commit>;剩下未勾的人判
9. push → gh pr create → flow-pr-merge <PR> --background(日志路径打印出来,合并结果最后核一眼)
10. 与 9 重叠做:memory(号段 / 门 / 进度 / 下一批从 N 起)+ 状态档轮数账补齐
```

## 轮数账模板(每批照填;固定开销单列,才看得出下次该砍哪)
